...
class Battleship(Environment):
    ...
    def animate(self,
                states: Sequence[BattleShipState],
                interval: int = 500,
                save_path: Path = None):
        fig, ax = plt.subplots()
        plt.close(fig)
        ...
        def make_frame(idx: int) -> None:
            ax.clear()
            state = states[idx]
            self.get_img(state, ax=ax, return_image=False)
    ...
    @staticmethod
    def get_img(state: BattleShipState,
                ax: plt.Axes = None,
                return_image: bool = False):
        hits_misses = state.hits_misses
        board = state.board

        rows, cols = hits_misses.shape
        if ax is None:
            fig, ax = plt.subplots()
        ...
        # Plot the hits and misses
        cax = ax.matshow(hits_misses, cmap=cmap, norm=norm)
        ...
        # Display ship locations with bigger black circles
        for i in range(rows):
            for j in range(cols):
                if board[i, j] == 1:  # Assuming 1 represents a ship part
                    ax.plot(j, i, 'ko', markersize=15)

        # plt.title('Battleship Game State')

        if return_image:
            # Save the plot to a BytesIO object and return the image
            buf = BytesIO()
